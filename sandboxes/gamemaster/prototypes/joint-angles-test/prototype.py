...
_relay_spec = importlib.util.spec_from_file_location(
    "joint_angles_relay_client", os.path.join(HERE, "relay_client.py"))
...
@bp.route("/api/config")
def config():
    # relay_host_default: the relay runs on this same hub, so the UI pre-fills
    # its host field with this server. identity tells the UI whether this is a
    # locked player copy (relay-only, own color) or the gamemaster copy (direct
    # mode + side choice available).
