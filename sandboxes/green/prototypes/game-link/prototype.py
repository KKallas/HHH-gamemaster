...
# What we poll on the host. The relay is a gamemaster-sandbox machine, so its
# mount includes the sandbox prefix.
RELAY_STATE_PATH = "/s/gamemaster/p/dobot-mg400-relay/api/state"
...
# ---- hub integration --------------------------------------------------------
# The hub hands us a HubContext via the optional hub_init hook: which sandbox we
# live in (= our team), the hub's own base URL, and the service token that
# authenticates our relay polls through the hub's auth layer. Stays None in a
# plain hub without that hook.
_hub_ctx = None


def hub_init(ctx):
    """Default the selection to what the hub already knows — this sandbox IS a
    team, and the gamemaster runs on this very server — unless game-link.json
    already pinned a choice. The relay enforces side == team server-side either
    way; this only sets the right default."""
    global _hub_ctx, _team, _host
    _hub_ctx = ctx
    try:
        with open(LINK_PATH) as f:
            saved = json.load(f)
    ...
    if not isinstance(saved, dict):
        saved = {}
    with _link_lock:
        if saved.get("team") not in TEAMS and ctx.sandbox in TEAMS:
            _team = ctx.sandbox
        host = saved.get("host")
        if not (isinstance(host, str) and host.strip()):
            _host = ctx.local_base
    _live.bump()
...
def _fetch_relay_state(host):
    ...
    ctx = _hub_ctx
    headers = {"X-HHH-Auth": ctx.service_token} if ctx is not None else {}
...
def _status_dict():
    ...
    arms[team] and sides[team]."""
    with _link_lock:
        team, host = _team, _host
    with _poll_lock:
        state, reachable, error = _relay_state, _reachable, _last_error
    your_arm_connected = your_arm_enabled = you_control = lease_secs = None
    if state is not None:
