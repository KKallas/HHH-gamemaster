...
def enable():
    ...
    with _arb_lock:
        if _estop:
            _log_command("robot", side, "enable", {}, ok=False, error="E-STOP latched")
...
def disable():
    ...
    data = request.json or {}
    _log_incoming("disable", data)
    side = _side_from(data)
    if side is None:
        _log_incoming("disable", data, ok=False, error="side must be 'purple' or 'green'")
        return _fail("side must be 'purple' or 'green'")
    robot = _arm(side)
    if robot is None or not robot.is_connected():
        _log_command("robot", side, "disable", {}, ok=False, error="Not connected")
        return _fail("Not connected")
    robot.stop_servo()
    try:
        errid, resp = robot.disable()
    except DobotError as e:
        _log_command("robot", side, "disable", {}, ok=False, error=str(e))
        return _fail(str(e), errid=e.errid)
    _log_command("robot", side, "stop_servo + disable", {"errid": errid, "resp": resp}, ok=(errid == 0))
    _live.bump()
    return jsonify({"ok": errid == 0, "errid": errid, "resp": resp, "side": side})
...
def clear():
    ...
    again on both sides."""
    global _estop
    _log_incoming("clear", request.json or {})
    with _arb_lock:
        _estop = False
    errors = {}
    for s in SIDES:
        ...
        if robot is not None and robot.is_connected():
            try:
                robot.clear_error()
                _log_command("robot", s, "clear_error", {}, ok=True)
            except DobotError as e:
                errors[s] = str(e)
                _log_command("robot", s, "clear_error", {}, ok=False, error=str(e))
    _live.bump()
    if errors:
...
def estop():
    ...
    global _estop
    _log_incoming("estop", request.json or {})
    with _arb_lock:
        _estop = True
    for s in SIDES:
        robot = _arm(s)
        if robot is not None and robot.is_connected():
            ...
            try:
                ...
                _log_command("robot", s, "stop_servo + emergency_stop", {}, ok=True)
            except DobotError:
                _log_command("robot", s, "emergency_stop", {}, ok=False, error="DobotError")
    _live.bump()
    return _ok()
...
def move():
    ...
    with _arb_lock:
        if _estop:
            _log_command("robot", side, "move", {}, ok=False, error="E-STOP latched")
...
def pump():
    ...
    with _arb_lock:
        if _estop:
            _log_command("robot", side, "pump", {}, ok=False, error="E-STOP latched")
            return _fail("E-STOP latched")
